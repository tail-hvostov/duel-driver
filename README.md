...
### /dev/duel2
Запись и чтениие, осуществляемые над этим файлом производятся в сыром режиме. В duel2 данные соответствуют нескольким последовательным блокам памяти, представляющим строки дисплея в порядке сверху-вниз.
## Тестирование
...
1. Начальная подготовка тестов (выполняется однократно). Выполните команду: ```scripts/duel_compile_tests.sh```
