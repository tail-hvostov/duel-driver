# duel-driver
## Описание проекта
Задачей проекта Duel является написание драйвера для монохромного SPI-дисплея, использующего контроллер SSD1306. Целевой платформой проекта является одноплатный компьютер OrangePi Zero 3 с ОС Armbian Linux 6.6.62-current-sunxi64.
## Установка дисплея
### Поддерживаемые дисплеи
Проект поддерживает дисплеи на основе контроллера SSD1306, использующие шину SPI и имеющие разрешение 72x40 или 128x64.
...
Выполните следующие команды в Терминале (в первой команде подстроку ```WxH``` следует заменить на запись, соответствующую разрешению используемого дисплея: ```72x40``` или ```128x64```):
```
sudo scripts/duel_overlay.sh WxH
...
Также при загрузке Duel в директории /proc порождается файл ```/proc/duel-params```, предназначенный для проверкии некоторых свойств дисплея.
...
### /dev/duel0
Запись и чтение, осуществляемые над этим файлом производятся в строковом режиме. Duel позволяет выводить на дисплей ограниченное множество символов из таблицы ASCII. Duel не поддерживает обработку специальных символов (например символов возврата каретки, переноса строки и т.п). Буфер строкового файла представлен массивом однобайтовых символов. Данный массив не использует нуль-терминатор или какой-либо иной способ обозначения длины записанного текста. Размер буфера вычисляется автоматически на основе параметров установленного дисплея. В начальном состоянии буфер текста заполнен пробельными символами. Буфер текста сбрасывается до начального состояния при создании дескриптора /dev/duel0, если до этого производилась сырая запись через /dev/duel1 или /dev/duel2. /dev/duel0 поддерживает системный вызов llseek.
### /dev/duel1
...
### /proc/duel-params
Данный файл доступен только для чтения. При его чтении возвращаются свойства дисплея в формате ключ/значение (например, ```width: 72```).
## Тестирование
